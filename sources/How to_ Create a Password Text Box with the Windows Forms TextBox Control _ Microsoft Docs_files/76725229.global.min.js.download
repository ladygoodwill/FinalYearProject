(function() {
    ...
    function getProfileUrl(username, locale) {
        return isNewSignIn ? "https://" + techProfileUrl + "/" + locale + "/" + username : "/profile";
    }
    ...
    var Dialog = function() {
        ...
        Dialog.prototype.show = function(view, options) {
            ...
            this.wrapper.addEventListener("click", function(event) {
                var target = event.target;
                if (target.closest(".dialog-close") || target.closest(".modal-overlay") || target.closest(".modal-close") || target.closest(".modal-background")) {
                    event.preventDefault();
                    _this.hide();
                }
            });
    ...
    function populateBreadcrumb(breadcrumbHolder, title, module) {
        ...
        switch (pageKind) {
          ...
          case "browse":
            breadcrumbHolder.innerHTML = template$2("Docs", "/" + userLocale + "/") + template$2("Learn", "/" + userLocale + "/learn/") + template$2(loc.browse);
            break;

          case "path":
            breadcrumbHolder.innerHTML = template$2("Docs", "/" + userLocale + "/") + template$2("Learn", "/" + userLocale + "/learn/") + template$2(loc.browse, "/" + userLocale + "/learn/browse/") + template$2(title);
            break;

          case "module":
            breadcrumbHolder.innerHTML = template$2("Docs", "/" + userLocale + "/") + template$2("Learn", "/" + userLocale + "/learn/") + template$2(loc.browse, "/" + userLocale + "/learn/browse/") + singleLearningPath + template$2(title);
            break;

          case "unit":
            breadcrumbHolder.innerHTML = template$2("Docs", "/" + userLocale + "/") + template$2("Learn", "/" + userLocale + "/learn/") + template$2(loc.browse, "/" + userLocale + "/learn/browse/") + singleLearningPath + template$2(module.title, "/" + userLocale + module.url) + template$2(title);
            break;
    ...
    function contentBrowserPage() {
        ...
        contentLoaded.then(function() {
            ...
            populateBreadcrumb(breadcrumbs, loc.browse);
    ...
    function learningPathPage() {
        ...
        Promise.all([ loadLearningPathInformation(pathId), contentLoaded ]).then(function(_a) {
            ...
            var breadcrumbs = document.querySelector(".breadcrumbs");
            populateBreadcrumb(breadcrumbs, learningPath.title);
    ...
    function displayRelatedLearningPaths(parents) {
        var template = function(paths) {
            return '\n\t<h2 class="title is-size-6 has-margin-top-medium has-margin-bottom-none">' + loc.relatedLearningPaths + '</h2>\n\t<ul class="has-margin-top-none has-margin-left-none has-margin-right-none">\n\t\t' + paths.map(function(path) {
    ...
    function renderSandboxDisabled(container) {
        var taskButtons = document.getElementById("task-button-group");
        if (taskButtons) {
            taskButtons.classList.add("is-hidden");
        }
        container.setAttribute(stateAttr, state$1.disabled);
        container.innerHTML = "\n\t\t\t<p>" + loc.sandboxTemporaryDisabled + "</p>";
        return new Promise(function() {});
    }
    ...
    function renderKnownError(container, error) {
        ...
        switch (error.errorCode) {
          ...
          case "OperationFailed":
            container.innerHTML = "<p>" + (error.message ? escape$1(error.message) : loc.somethingWentWrong) + "</p>";
            return new Promise(function() {});
